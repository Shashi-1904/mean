...
export class ContactUsService {
  ...
  private apiUrl = 'http://localhost:4200/api/contactUs/';
  ...
  sendReplyMessage(replyMsg: any) {
    return this.httpClient.post<any>(`http://localhost:4200/api/contactReply/addContactReply`,replyMsg).pipe
    (
      switchMap(({msg}) => {
        return of(msg);
      }),
  ...
  replyMessageHistory() {
    return this.httpClient.get<any>(`http://localhost:4200/api/contactReply/contactReplyHistory`).pipe
    (
